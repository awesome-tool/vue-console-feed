<template>
  <div class="console-colors console-wrap console-item" :class="type ? `console-${type}` : undefined" ref="elRef">
    <span v-if="count && (Number(count) > 1)" class="console-icon console-badge">{{
      count
      }}</span>
    <span v-else-if="type && type !== 'log'" class="console-icon" />

    <div class="console-value" :class="{
      'has-location': location
    }">
      <LocationConsole v-if="location" class="truncate console-location" :location="location" :anchor="Anchor" />
      <div class="console-message">
        <template v-if="Array.isArray(data)">
          <ConsoleValue v-for="(item, index) in data" :key="index" :data="item" :is-log="type !== undefined"
            :_get-list-link-async="_getListLinkAsync ?? _getListLinkAsyncDefault
              " :read-link-object-async="readLinkObjectAsync ?? readLinkObjectAsyncDefault
                " :call-fn-link-async="callFnLinkAsync ?? callFnLinkAsyncDefault" :anchor="Anchor" class="mr-1" />
        </template>
        <ConsoleValue v-else :data="(data as EncodeData)" :is-log="type !== undefined"
          :_get-list-link-async="_getListLinkAsync ?? _getListLinkAsyncDefault" :read-link-object-async="readLinkObjectAsync ?? readLinkObjectAsyncDefault
            " :call-fn-link-async="callFnLinkAsync ?? callFnLinkAsyncDefault" :anchor="Anchor" />
      </div>
    </div>
  </div>
</template>
